    ...
    if (this.props.question.questionStatus === questionStatus.notAnswerYet) {
      msg = (
        <div>
          ...
          <p
            choice="1"
            onClick={e => this.props.handler(e, 0)}
            className="msgFromMe choice"
          >
            {lines.get(this.props.question.choices[0]).question}
          </p>
          <p
            choice="2"
            onClick={e => this.props.handler(e, 1)}
            className="msgFromMe choice"
          >
            {lines.get(this.props.question.choices[1]).question}
          </p>
          <p
            choice="3"
            onClick={e => this.props.handler(e, 2)}
            className="msgFromMe choice"
          >
            {lines.get(this.props.question.choices[2]).question}
          </p>
        </div>
      );
    } else {
